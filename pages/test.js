...
export function championIdToName({ championId }) {
  switch(championId){
...
export default function Test({ posts,postsi }) {
  ...
  postsi.championId = 2
  console.log(postsi)
  let championName = championIdToName(postsi.championId)
    ...
    return (
      <div>
        ...
        <ul>
          ...
          <li> {championName} avec {postsi.championPoints} points</li>
        </ul>
      </div>
  ...
  export async function getServerSideProps({ query }) {
    ...
    const res = await fetch('https://euw1.api.riotgames.com/lol/summoner/v4/summoners/by-name/'+id+'?api_key=RGAPI-5edb0ed7-ee1a-4a01-af04-88e5dc02b2ca')
    const posts = await res.json()

    const resi = await fetch('https://euw1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/'+posts.id+'?api_key=RGAPI-5edb0ed7-ee1a-4a01-af04-88e5dc02b2ca')
